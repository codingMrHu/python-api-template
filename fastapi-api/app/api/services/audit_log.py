...
class AuditLogService:
    ...
    def insert_user(cls, user: User, ip_address: str, note: str = '新建用户'):
        ...
        """
        logger.info(
            f"act=update_system_user user={user.user_name} ip={ip_address} user_id={user.id} note={note}")
    ...
    def user_login(cls, user: User, ip_address: str):
        logger.info(
            f"act=user_login user={user.user_name} ip={ip_address} user_id={user.id}")
        # 获取用户所属的分组
        cls._system_log(user, ip_address, EventType.USER_LOGIN,
                        ObjectType.NONE, '', '')
